...
- **Poprawki exemplarza/README Case 3 Lekcji 4 i metodologii preprocessingu Capstone** — dostosowanie `impute_missing`/`split_dataset`/ewaluacji klasteryzacji do konwencji Case 3: stratyfikowany split klasyfikacji, `cluster_stability` przeniesiony z Case 3, scaler zwracany z `scale_features` (#39)
...
- **Dwie kolejne instancje tej samej krucho ści** — testy `check.py` asercjonujące właściwość klastra względem konkretnego numeru etykiety KMeans, dla którego sklearn nie gwarantuje kolejności między wersjami — znalezione i naprawione po tym, jak pierwsze podejście z #47 je przeoczyło, zamykając tę klasę błędu w całym repo (#50, #51)
