...
Pięć powiązanych wzorców, wszystkie realne, wszystkie złapane w tym samym audycie i naprawione w jednym PR-ze (#44):
...
- **Podanie kosztu operacyjnego jako faktu bez podstawy w briefie czy danych** — wzorcowa notatka Case 2 pierwotnie stwierdzała, że fałszywy alarm "kosztuje tylko kilka minut", jakby to było dane z góry, mimo że nic w briefie klienta tego nie potwierdzało — naprawa przeformułowuje to jako założenie do potwierdzenia z klientem i dodaje realny wolumen, którego ta decyzja dotyczy (45 ze 140 zamówień testowych oznaczonych, ~32%), żeby realna stawka założenia była widoczna.
- **Mylenie stabilności klastrów opartej na resamplingu z wrażliwością na inicjalizację K-means** — test stabilności, który zmienia tylko *próbkę* (trzymając `random_state` na stałe), nic nie mówi o tym, czy inna losowa inicjalizacja znalazłaby te same klastry. To różne pytania z różnymi odpowiedziami — Case 3.
