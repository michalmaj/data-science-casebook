...
- **Case 3 Lesson 4 exemplar/README fixes and Capstone preprocessing methodology** brought the capstone's `impute_missing`/`split_dataset`/clustering evaluation in line with Case 3's own conventions — stratified classification split, `cluster_stability` ported from Case 3, scaler returned from `scale_features` (#39)
