...
This note earns "Exemplary" on **Interpretation and limitations** by naming the test-set-reuse issue explicitly and explaining its practical consequence (numbers may be slightly optimistic) rather than glossing over it — that's exactly the kind of self-aware limitation the rubric rewards, and it's more valuable than a limitations section that only lists generic caveats. It earns "Exemplary" on **Communication** in Section 5 by translating "55% recall, 24% precision" into a concrete "1 in 4 flagged orders is real" statement a non-technical stakeholder can act on.
